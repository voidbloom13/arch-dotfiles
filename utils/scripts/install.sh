...
function gnome_setup {
  sudo pacman -Syu gnome gnome-tweaks extension-manager
}

function hypr_setup {
  # Hyrpland-specific Packages
  sudo pacman -S blueman brightnessctl hypridle hyprlock hyprpaper networkmanager nm-connection-editor nwg-dock-hyprland pipewire swaync waybar wireplumber
  yay -S hyprshot mirage
...
function dotfiles_setup {
  ...
  read -p "Would you like to import dotfiles? [Y/n] " dotfiles_setup
  dotfiles_setup=${dotfiles_setup:-Y}
  case $dotfiles_setup in
    [yY] )
      echo "Importing dotfiles now..."
      git clone https://github.com/voidbloom13/arch-dotfiles $HOME/repo_dotfiles
      rsync -av --backup --backup-dir="$HOME/.dotfiles/backups" --suffix=".bak_$(date +%F_%H:%M:%S)" $HOME/repo_dotfiles/.dotfiles/ $HOME/Documents/Test # Change this to $HOME/.dotfiles
      rm -rf $HOME/repo_dotfiles
...
function main {
  ...
  cd $HOME
  sudo pacman -Syu aspnet-runtime base-devel cifs-utils curl discord docker dotnet-runtime dotnet-sdk fastfetch flatpak fzf gcc ghostty git github-cli jdk-openjdk krita libreoffice-fresh man maven nvim nodejs npm obsidian pgcli postgresql ripgrep rsync stow tailscale tmux tree ttf-font-awesome $(pacman -Sgq nerd-fonts) unzip zip zoxide zsh
  ...
  yay -S google-chrome opentabletdriver montserrat-otf
